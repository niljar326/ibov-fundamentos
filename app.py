...
with tab1:
    st.markdown("""<div style="text-align: justify; margin-bottom: 20px;">Este <b>Screener Fundamentalista</b> filtra automaticamente as melhores oportunidades. Abaixo, você também encontra uma lista de <b>Alerta</b> para empresas em situações delicadas.</div>""", unsafe_allow_html=True)
    if not df_best.empty:
        ...
        styler = df_best[cols_view].style.map(lambda x: 'background-color: #f2f2f2; color: black;', subset=['Preço', 'P/L', 'DY']).format({"Preço": "R$ {:.2f}", "EV/EBIT": "{:.2f}", "P/L": "{:.2f}", "ROE": "{:.2f}", "DY": "{:.2f}", "Margem Líq.": "{:.2f}"})
    ...
    with col_ad1: st.markdown("""<div style="background-color: #fffbe6; border: 1px solid #ffe58f; padding: 15px; border-radius: 10px; color: #333; height: 100%;"><h4 style="margin-top:0; color: #333;">✈️ Nomad: Taxa Zero em Dólar</h4><p style="font-size: 14px;">Ganhe taxa zero na 1ª conversão (até US$ 1.000) para investir nos EUA.</p><p style="font-size: 14px;">Código: <code style="background-color: #eee; padding: 4px; border-radius: 4px; border: 1px solid #ddd; font-weight:bold;">Y39FP3XF8I</code></p><div style="text-align:center;"><a href="https://nomad.onelink.me/wIQT/Invest?code=Y39FP3XF8I%26n=Jader" target="_blank" style="text-decoration: none; color: white; background-color: #1a1a1a; padding: 10px 15px; border-radius: 5px; font-size: 14px; display: inline-block; width: 100%;">➡️ <b>Abrir Conta Nomad</b></a></div></div>""", unsafe_allow_html=True)
    with col_ad2: st.markdown("""<div style="background-color: #eaf6ff; border: 1px solid #bae0ff; padding: 15px; border-radius: 10px; color: #333; height: 100%;"><h4 style="margin-top:0; color: #009ee3;">🤝 Mercado Pago: R$ 30 OFF</h4><p style="font-size: 14px;">Use o app pela primeira vez (pagamento mín. R$ 70) e ganhe <b>R$ 30 de desconto</b>.</p><div style="text-align:center;"><a href="https://mpago.li/1VydVhw" target="_blank" style="text-decoration: none; color: white; background-color: #009ee3; padding: 10px 15px; border-radius: 5px; font-size: 14px; display: inline-block; width: 100%;">➡️ <b>Resgatar R$ 30</b></a></div></div>""", unsafe_allow_html=True)
    st.divider()
    ...
    else: st.info("Nenhuma ação com os critérios de risco encontrados hoje.")
    st.divider()
...
with tab2:
    ...
    st.warning("⚠️ **Atenção:** Este filtro mostra ações tocando a banda inferior.")
    col_list, col_chart = st.columns([1, 2])
    with col_list:
        if not df_scan_bb.empty:
            st.write(f"**{len(df_scan_bb)} Oportunidades:**")
            event = st.dataframe(df_scan_bb[['Ativo', 'Preço Atual', 'Distância Fech %']].style.format({"Preço Atual": "{:.2f}", "Distância Fech %": "{:.2f}%"}).map(lambda x: 'background-color: #ffcccb; color: black', subset=['Distância Fech %']), use_container_width=True, hide_index=True, on_select="rerun", selection_mode="single-row")
            if len(event.selection.rows) > 0:
                selected_index = event.selection.rows[0]
                st.session_state.tv_symbol = df_scan_bb.iloc[selected_index]['TV_Symbol']
        else: st.info("Nenhuma ação brasileira encontrada tocando a banda inferior nesta semana.")
    with col_chart:
...
with tab3:
    # 1. LINK DE PROPAGANDA DEFINIDO AQUI
    url_anuncio = "https://www.effectivegatecpm.com/cem06mi8m?key=43a0b19ca2224de6776045a258cdf621"

    # 2. INICIALIZAÇÃO SEGURA DO ESTADO (Começa Bloqueado)
    ...
    if not st.session_state["conteudo_roc_liberado"]:
        ...
        with col_lock2:
            ...
            st.info("ℹ️ Para acessar este conteúdo gratuitamente, solicitamos que você **clique no botão abaixo**. Você será redirecionado para uma página de publicidade de nossos parceiros que ajuda na manutenção dos custos do site.")
            
            # BOTÃO DE AÇÃO
            if st.button("🔓 CLIQUE AQUI PARA APOIAR E LIBERAR", type="primary", use_container_width=True):
                # 1. Injeta o JS para abrir a nova aba ANTES de mudar o estado visualmente
                js = f"<script>window.open('{url_anuncio}', '_blank');</script>"
                components.html(js, height=0)
                
                # 2. Muda o estado para liberado
                st.session_state["conteudo_roc_liberado"] = True
                
                # 3. Recarrega a página para remover o bloqueio
                st.rerun()
