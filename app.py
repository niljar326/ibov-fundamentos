...
def scan_bollinger_bands_weekly():
    tickers_br = [
        ...
        "TOTS3.SA", "RENT3.SA", "TIMS3.SA", "SBSP3.SA", "ELET3.SA"
    ]
        ...
        for t in all_tickers:
            try:
                ...
                if curr['Low'] <= curr['Lower']:
                    setup_detectado = "Banda Inferior"
                
                # 2. Tocou na Média Central (Pullback/Suporte)
                # Condição: Mínima tocou na média (Low <= SMA20) E Máxima está acima (High >= SMA20)
                # Isso indica interação com a média
                elif (curr['Low'] <= curr['SMA20']) and (curr['High'] >= curr['SMA20']):
                    setup_detectado = "Média Central (Pullback)"

                if setup_detectado:
                    candidates.append({
                        'Ativo': clean_ticker,
                        'Mercado': '🇧🇷' if ".SA" in t else '🇺🇸',
                        'Setup': setup_detectado,
                        'Preço Atual': curr['Close'],
                        'Alvo/Suporte': curr['Lower'] if setup_detectado == "Banda Inferior" else curr['SMA20'],
...
with tab2:
    st.subheader("📉 Estratégia: Tocou na Banda Inferior ou Média (Semanal)")
    st.markdown("""
    Lista rastreada automaticamente de ações Brasileiras e Americanas onde:
    1. A **Mínima da Semana** tocou a **Banda Inferior**.
    2. OU a **Mínima da Semana** tocou a **Média Central** (Basis).
    ...
    with col_list:
        if not df_scan_bb.empty:
            ...
            def color_setup(val):
                if val == "Banda Inferior": return 'background-color: #ffcccb; color: black' # Vermelho
                return 'background-color: #e6fffa; color: black' # Verde para Média
            
            # Tabela Interativa
            event = st.dataframe(
                df_scan_bb[['Ativo', 'Mercado', 'Setup', 'Preço Atual', 'Alvo/Suporte']].style.format({
                    "Preço Atual": "{:.2f}", "Alvo/Suporte": "{:.2f}"
            ...
            st.info("Nenhuma ação tocando a banda inferior ou média central nesta semana.")
